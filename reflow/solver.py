    ...
    with torch.no_grad():
        for bidx, all_data in enumerate(loader_test):
            ...
            mel = model(
                    ...
                    data['f0'], 
                    data['volume'], 
                    timbre_data['spk_embedding'],
                    timbre_data['spk_embedding_rev'],
                    vocoder=vocoder,
                    infer=True,
                    return_wav=False,
    ...
    for epoch in range(start_epoch, args.train.epochs):
        for batch_idx, all_data in enumerate(loader_train):
            ...
            if dtype == torch.float32:
                loss = model(data['units'].float(), data['wenet'].float(), data['whisper'].float(), data['f0'], data['volume'], timbre_data['spk_embedding'], timbre_data['spk_embedding_rev'], 
                                aug_shift=data['aug_shift'], vocoder=vocoder, gt_spec=data['mel'].float(), infer=False)
            else:
                if use_torch_musa:   
                    with torch.musa.amp.autocast(dtype=dtype):
                        loss = model(data['units'], data['wenet'], data['whisper'], data['f0'], data['volume'], timbre_data['spk_embedding'], timbre_data['spk_embedding_rev'],
                                        aug_shift=data['aug_shift'], vocoder=vocoder, gt_spec=data['mel'].float(), infer=False)
                else:
