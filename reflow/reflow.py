...
class Bi_RectifiedFlow(nn.Module):
    ...
    def reflow_loss(self, x_1, x_0, t, cond=None, loss_type='l2_lognorm', return_pred=False):
        # 1. 构造中间状态 x_t
        ...
        # 3. 计算常规的 Flow Matching Loss (保证 ODE 轨迹正确)
        if loss_type == 'l1':
        ...
        # 4. 【新增】推导预测的最终频谱 X_1_pred
        # 公式: X_1 = X_t + (1-t) * v
        if return_pred:
            x_1_pred = x_t + (1 - t[:, None, None, None]) * v_pred
            return loss, x_1_pred
    ...
    def forward(self, 
                ...
                method='euler',
                use_tqdm=True,
                t_input=None, return_pred=False):
        if cond is not None:
...
# 【新增】支持外部传入 t，保证两个流的时间步同步
            ...
            return self.reflow_loss(x_1, x_0, t, cond=cond, return_pred=return_pred)
        else:
